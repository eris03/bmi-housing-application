...
(function (root) {
  ...
  function sanitize(s) {
    ...
    return String(s)
      // Newlines/tabs/control chars can't be encoded by the standard (WinAnsi)
      // font and make pdf-lib throw mid-build — pressing Enter inside an address
      // textarea is the usual cause. Collapse them to a single space first.
      .replace(/[\r\n\t\f\v]+/g, ' ')
      .replace(/₹/g, 'Rs.')
      .replace(/[‘’]/g, "'")
      .replace(/[“”]/g, '"')
  ...
  async function fillPdf(PDFLib, templateBytes, data, images, mode) {
    ...
    if (serial) draw(p1, font, serial, 480, 708, 13, red); // Sl. No. (red)
    // Site size + layout name are written ABOVE the two dotted blanks on the
    // SAME line ("…site Measuring [size] in [layout]"): baseline raised ~5pt
    // over the printed-text baseline (y=579.9) so each value rests on top of
